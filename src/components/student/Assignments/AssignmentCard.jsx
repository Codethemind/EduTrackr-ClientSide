...
const AssignmentCard = ({ assignment, onView, onSubmit }) => {
  ...
  const isDueSoon = dueDate <= new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000) && dueDate > now && !assignment.hasSubmitted;
  ...
  const getTimeRemainingText = () => {
    if (assignment.hasSubmitted) {
      return `Submitted on ${new Date(assignment.submission?.submittedAt).toLocaleDateString()}`;
    } else if (isOverdue) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
      ...
      <div className="mb-4">
        <div className={`text-sm font-medium ${
          assignment.hasSubmitted ? 'text-green-600' :
          isOverdue ? 'text-red-600' :
          isDueSoon ? 'text-yellow-600' : 'text-blue-600'
        }`}>
      ...
      <div className="flex gap-2">
        ...
        {!assignment.hasSubmitted && !isOverdue && (
          <button
